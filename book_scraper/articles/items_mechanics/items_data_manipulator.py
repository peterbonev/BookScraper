...
class ItemsDataManipulator(ItemsLinkData):
    ...
    def sort_data(self, key, order=False):
        ...
        """
        if self.is_search_items_empty() or not self.is_key_valid(
            self.searched_items[0], key
        ):
            return
        lists = sorted(
            self.gen_values_searched_items(),
            key=lambda k: (k[key], k[self.default_key]),
            reverse=order,
        )
    ...
    def filter(self, key=None, value=None):
        ...
        self.searched_items = [
            ...
            if str(value).lower() in str(item[key]).lower()
        ]
