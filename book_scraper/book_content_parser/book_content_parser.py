...
from book_scraper.book_content_parser.book_content_parser_filter import (
    BookDataParserFilter,
)
...
class BooksContentParser(DataParserMixin, BookDataParserFilter):
    ...
    def collect(
        self, verified_input=None, args=None, num_books=None
    ):  # previous def scrape_all_info
        """
        ...
        for page in page_responses:
            ...
            if self._exclusion_selection:
                if self._exclude(self._exclusion_selection, book_info):
                    ...
                    results.add(
                        title, genre, price, rating, num_available, description, upc
                    )
            else:
                results.add(
                    title, genre, price, rating, num_available, description, upc
