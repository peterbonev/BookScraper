...
from book_scraper.book_link_parser.book_link_parser_defines import (
    DOMAIN_ALL_GENRES,
    A_TAG,
    D_TAG,
    DOMAIN_NAME,
...
class BookGenresParser(object):
    ...
    def _parse_genres():
        ...
        selection = (
            soup.find(D_TAG, DOMAIN_ALL_GENRES).get_text(",", strip=True).split(",")[1:]
        )
        all_genres_raw_data = soup.find(D_TAG, DOMAIN_ALL_GENRES).find_all(
            "a", href=True
        )[1:]
        ...
        for index in range(0, len(all_genres_raw_data)):
            links.add(
                selection[index],
                "{}{}".format(DOMAIN_NAME, all_genres_raw_data[index]["href"]),
            )  #'{}{}'.format(DOMAIN_NAME, link['href'])

        return links

    @staticmethod
    def _parse_by_genre(
        input_list_genres, links_object, limit=None
    ):  # old: parse_url_genre_cat
        """
        ...
        for raw_data in links_object.content().keys():
            if not raw_data in input_list_genres:
                ...
                result_links.add(
                    "{}{}".format(raw_data, 0),
                    "{}{}{}".format(
                        links_object.content()[raw_data][:FROM_END],
                        "index",
                        PAGE_SUFFIX,
                    ),
                )
        ...
        for request in requests.content():
            ...
            for url in input_tag:
                result_links.add(
                    ...
                    "{}{}".format(
                        DOMAIN_NAME_STRING,
                        re.sub("^(../)+(..)/", "", url.find(A_TAG).get(HREF_TAG)),
                    ),
                )
                index += 1
