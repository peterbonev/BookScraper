...
class BookLinkAllBooksParser(object):
    ...
    def _parse_all_urls(self, target_amount=None):
        ...
        """
        all_links = Links()
        generate_url_guess_list = [
            "{}{}{}".format(DOMAIN_URL_PREFIX, index, PAGE_SUFFIX)
        ...
        if len(requests.content()) == MAX_VALUE:
            generate_url_guess_list = [
                "{}{}{}".format(DOMAIN_URL_PREFIX, index, PAGE_SUFFIX)
                for index in range(1, MAX_VALUE * 2)
            ]
            requests = RequestsParser(generate_url_guess_list)
        index = 0
        for request in requests.content():
            ...
            for url in input_tag:
                all_links.add(
                    index,
                    "{}{}".format(DOMAIN_NAME_STRING, url.find(A_TAG).get(HREF_TAG)),
                )
                index += 1
                if target_amount:
