from book_scraper.book_link_parser.book_link_all_books_parser import BookLinkAllBooksParser
...
class BookLinkParser(object):
    ...
    def collect(self, list_genres=None, num_books=None):
        ...
        """
        result = Links()
        if (num_books and not list_genres) or (len(list_genres) == len(self.__genre_parser._parse_genres().content())):
            result = self.__book_parser._parse_all_urls(num_books)
        else:
            result = self.__genre_parser._parse_by_genre(list_genres, self.__genre_parser._parse_genres(), num_books)
