...
class CLIInputParser(object):
    ...
    def __init__(self):
        ...
        self._parser = argparse.ArgumentParser(description='''The following arguments can be used:\n
                                   b - number of books
        ...
        self._parser.add_argument('-X', dest='X', action='store_const', const=True, default=False)
        self._parser.add_argument('-b', type=int, metavar="[0-1000]", default=1000, nargs='?', dest='books')
        self._parser.add_argument('-g', nargs='+', dest='genres')

        self.__group1 = self._parser.add_argument_group()
        self.__group1.add_argument('-s', type=str, nargs='?', choices=[None, 'price', 'rating', 'available', 'title'],
                                   dest='sort')
        self.__group1.add_argument('sort_type', nargs='?', choices=[None, 'ascending', 'descending'])
