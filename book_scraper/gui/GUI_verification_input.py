    ...
    def verify(self, verified_data):
        ...
        if int(self._books.get()) > 0:
            ...
            tkMessageBox.showinfo(
                "Wrong Input Error", "Wrong books value. Expected value positive int"
            )
        verified_data.genres = self._genres.get(0, END)

        if int(self._filter[3].get()):
            if int(self._filter[2].get()) > 0:
                verified_data.filter = [
                    self._filter[0].get(),
                    "{}{}".format(self._filter[1].get(), self._filter[2].get()),
                ]
            else:
                tkMessageBox.showinfo(
                    "Wrong Input Error",
                    "Wrong filter value. Expected value positive int",
                )
