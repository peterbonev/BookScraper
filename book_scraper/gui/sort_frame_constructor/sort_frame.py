...
class BookScraperApplicationSortFrame(object):
    ...
    def construct(self, frame):
        ...
        self._drop_filters_options = ttk.Combobox(
            self.sort_frame, value=self._sort_options
        )
        self._drop_filters_options.current(0)
        self._drop_filters_options.grid(
            row=10, column=1, padx=5, pady=5, ipadx=15, sticky=W
        )
